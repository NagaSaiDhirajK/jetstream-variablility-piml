...
def wind_divergence(
    ...
    """
    dy, dx = _grid_spacing_m(lat_deg, lon_deg)
    du_dx = torch.gradient(u_field, dim=2)[0] / dx[None, :, :]
    dv_dy = _safe_central_gradient(v_field, dy, dim=1)
    return du_dx + dv_dy
...
def divergence_residual_loss(
    ...
    """L_div = ||(du/dx + dv/dy) / reference_scale||^2."""
